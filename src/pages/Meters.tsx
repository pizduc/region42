...
const Meters = () => {
  ...
  const [previousReadings, setPreviousReadings] = useState<{[key: string]: number}>({});
  ...
  // Check if current date is within allowed range (20-25 of each month)
    ...
    const loadLatestReadings = async () => {
      try {
        ...
        if (data.currentReadings) {
          // Сохраняем предыдущие показания для статистики
          setPreviousReadings(data.currentReadings);
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    // Check if current date is within allowed period
    if (!isWithinAllowedPeriod()) {
      toast({
        title: "Период подачи показаний закрыт",
        description: "Показания можно подавать только с 20 по 25 число каждого месяца.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      // Обновляем предыдущие показания после успешной отправки
      setPreviousReadings({
        cold_water: payload.coldWater,
        hot_water: payload.hotWater,
        ...
        <div className="w-80 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm border-r border-gray-200 dark:border-gray-700 shadow-xl">
          <div className="p-6">
            ...
            <div className="space-y-4">
              <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wide">Предыдущие показания</h3>
              {meters.map((meter) => (
                <div key={meter.id} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                  ...
                  <span className="text-sm font-mono font-bold text-gray-900 dark:text-gray-100">
                    {previousReadings[meter.id] ? Number(previousReadings[meter.id]).toFixed(3) : '0.000'}
                  </span>
                </div>
              ))}
            </div>
            ...
            {!isWithinAllowedPeriod() && (
              <div className="mb-6 p-4 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg">
                <div className="flex items-center gap-2 text-yellow-800 dark:text-yellow-200">
                  ...
                  <span className="font-medium">Период подачи показаний: с 20 по 25 число каждого месяца</span>
                </div>
              </div>
            )}
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-3 gap-3">
                          <Button 
                            ...
                            disabled={!isWithinAllowedPeriod()}
                          >
                            +{meter.step * 100}
                          </Button>
                          <Button 
              ...
              <div className="xl:col-span-2 pt-4">
                <Button 
                  ...
                  disabled={isLoading || !isWithinAllowedPeriod()}
                >
                  {isLoading ? (
                    <div className="flex items-center gap-2">
                  ...
                  <div className="space-y-6">
                    ...
                    {previousReadings[meter.id] && (
                      <div className="text-center text-sm text-gray-600 dark:text-gray-400">
                        Предыдущие показания: <span className="font-mono font-bold">{Number(previousReadings[meter.id]).toFixed(3)}</span>
                      </div>
                    )}
